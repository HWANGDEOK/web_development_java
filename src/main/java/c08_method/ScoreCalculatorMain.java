...
public class ScoreCalculatorMain {
    public static void main(String[] args) {
        ...
        double score1 = 0;
        double score2;
        // double totalScore = 0 으로 선언 및 초기화하는 것은 필수이므로 표시한 것
        double totalScore=0;
        double avgScore = 0;
        boolean endOfCalc = false;

        // 1.기존 오버라이딩을 이용한 단순 연잔자 사용 및 메서드 활용 예제
...
//        System.out.println("총 합은 "+(score1 + score2)+"점 입니다.");
...
//        System.out.println("평균 점수는 "+avgScore+"점 입니다(메서드 활용).");
        ...
        // 2. 점수 누적적으로 더할 수 있게끔 작성하는 방식의 프로그래밍
        while(!endOfCalc){          // endOfCalc 를 추후 true로 바꾸는 조건을 통해 반복문 종료 예정
            System.out.print("점수를 입력하세요(종료하려면 -1을 입력하세요) : ");
            score1 = sc.nextDouble();
            if(score1 == -1){
//                break;      // switch문에서 배운 break문으로 : 반복문 종료할 때 쓰임 -> 곧장 반복문을 종료시키고자 할 때 사용
                endOfCalc = true;   // endOfCalc 변수를 true로 바꿔줌으로써 반복문 종료
            }else if(score1>=0){
                totalScore = scoreCalculator.calculateSum2(totalScore, score1);
                totalSubjects++;
            }
            // test code 삽입
//            System.out.println("총 점은 "+totalScore+"점 입니다.\n현재 과목의 수는 "+totalSubjects+"\n");
        ...
        // 분모가 0이면 예외가 발생하기 때문에, totalSubjects가 0 초과일 때만 실행되도록 조건문 설정 -> 예외처리 고전적 방법
