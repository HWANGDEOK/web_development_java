...
public class Method01 {
        /*
            ...
            Method : 일련의 동작을 묶어서 정의한 것, 입력값과 출려값이 존재(optional)

            함수와 메서드가 동일 개념이라면 왜 용어를 분리하는가?
                -> 클래스 내부에 함수를 정의하게 되면 이를 메서드라고 칭함.
                -> 자바의 경우는 모든 함수들이 특정 클래스에 종속돼있기 때문에 전부 다 메서드에 해당
            ...
            메서드 호출(call) : main method가 있는 곳에서 클래스명.메서드명() 혹은,
                ...
                미리 입력(메서드 정의)해놓고 놔서, '사진을 찍는다'라는 명령어를 실행(메서드 호출)시
    ...
    public static void main(String[] args) {
        // 1번 유형 호출 -> 아까 전에 scanner.nexLine()의 경우에는 객체명.메서드명()이라고 했는데
        // 왜 여기서 call1()만으로 호출이 가능한가?
//        call1();
//        // 2번 유형 호출
//        call2("잠자기");
//        // 사용할 클래스 표기
...
//        routine = sc.nextLine();  // -> "본가 방문"이라고 입력했다면, 그것은 scanner.nextLine()의 return값
...
//        System.out.println(todayDate);      // 실행 방법 2 : 별도의 변수를 선언하여 메서드의 return 값을 저장 후,
//                                            //  변수를 통해 sout 실행

        System.out.print("당신이 태어난 년도는 언제입니까? ");
        int year = sc.nextInt();
        System.out.print("당신이 태어난 월은 언제입니까? ");
        int month = sc.nextInt();
        System.out.print("당신이 태어난 일은 언제입니까? ");
        int date = sc.nextInt();
