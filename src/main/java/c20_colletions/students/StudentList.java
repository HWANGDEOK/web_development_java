...
//        // 0번지 부터 4번지까지 찾아넣었기 때문에 인덱스 넘버 3에 위치한 요소의 학번
...
//
//        // "박수빈" 이름을 가진 객체의 학번을 콘솔에 출력하시오. -? 검색과 관련있습니다. StringList.java 확인해서 만들 것
...
//        // 일반 for문
//        for(int i=0; i < students.size(); i++){
//            if(students.get(i).getId() == searchInt){   // int 냐 String에 따라 조건식 방식이 달라진 부분에 주목하세요
...
//        }
        ...
        for(Student student : students){
            if(student.getId() == 20240002){
                if(student.getName().equals("강준석")){
                    student.setName("박승주");
                    System.out.println(student.getId() + " 학번의 학생의 이름이 " + student.getName() + "으로 바뀌었습니다.");
                    break;
                }
            }
        }
        ...
        // 인덱스 넘버로 삭제해야하기 때문에 향상돈 for문의 사용이 불가능했습니다.
//        for(int i=0; i< students.size(); i++){
...
//        System.out.println("삭제 후 리스트 : " + students);
        ...
        // .remove(Object o)를 이용한 방법 -> 이 경우는 향상된 for 문도 가능합니다.
        for(Student student :students){
        ...
        System.out.println("삭제 후 리스트2 : " + students);
    }
}
