...
/*
    ...
    상속(inheritance)란?
        객체 지향 프로그래밍(OOP)의 핵심 개념 중 하나로 기존 클래스(부모 클래스, 슈퍼 클래스)의 속성과 메서드를 재사용하여 새로운 클래스(자식 클래스,
        서브 클래스)를 만드는 것을 의미
    ...
    1. 왜 상속이 필요한가?
        ...
        이전에는 Dog 클래스에 이름, 나이라는 필드를 선언하고, 걷기, 먹기 메서드를 정희한 후
        Cat 클래스에 이름, 나이 필드를 선언하고, 걷기, 먹기 메서드를 정의해야만 했다면,
        이제는 해당 부분에 대한 코드 중복을 줄일 수 있다는 점에서 상속의 의의가 있다.

    2. 상속의 특징
        1) 단일 상속 : 자바는 단일 상속만 지원함. 하나의 클래스는 한 번에 하나의 부모 클래스만 상속받을 수 있어 상속 구조가 간단하고 명확함
        2) super 키워드 : 클래스의 생성자 및 setter에서 학습한 this와 비슷한 개념으로 부모 클래스의 맴버에
    ...
    3. 상속의 장점
        ...
        2) 유지보수의 용이성 : 부모 클래스의 변경 사항이 자식 클래스에 반영되기 때문에 수정이 용이함.
        3) 객체 간 관계 표현 : IS-A 관게를 명확이 나타냄
