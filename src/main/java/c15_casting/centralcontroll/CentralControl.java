...
public class CentralControl {
    ...
    public void addDevice(Power device){
        // checkEmpty()는 좀 있다가 정의
        // 개발 환경에서는 만들다보면 추가적으로 메서드 구현이 필요한 경우가 있어
        // 일부러 checkEmpty()를 먼저 작성한 후 addDevice를 작성하는 게 아니라
        // 순서를 지금과 같이 하게됨
    ...
    public int checkEmpty(){    // 비어있는 배열의 index를 반환할 거기 때문에
        ...
        // 배열 내부를 탐색하여 비어있는 '첫 번째' 인덱스를 리턴할 에정
    ...
    // 배열 내부에 있는 elements 등의 전원을 전부 켜고, 끄는 메서드를 구현
    public void powerOn(){
        // 배열 내부에 있는 요소들(객체들 - Power의 자식 클래스들의 객체)의 공통된 메서든인 .on()을 실행시킬 예정
        /*
            해당 경우에 생겨날 수 있는 문제점 :
                특정 배열 인덱스 내에 아무 객체가 없다면 .on()을 적용시켰을 경우 컴파일 에러가 나기 때문에
                해당 부분을 감안한 코드를 작성합니다.
         */
        for(int i=0; i<deviceArray.length; i++){
            if(deviceArray[i] == null){
                // 특정 인덱스 넘버의 element가 null 이라면
                continue;       // 해당 반복문파트는 종료하고 다음 반복문 인덱스로 넘기겠다는 의미
            }
            deviceArray[i].on();    // continue가 실행되지 않았다면 해당 라인의 코드가 실행되도록 작성
        }
    }
    ...
    // 향상된 for문을 이용하여 powerOff2() 메서드를 구현하시오 -> 이휴 Main에서 powerOff2() 메서드 실행
    ...
    public void displayInfo2(){
        ...
        // 배열 내부의 요소를 하나씩 뽑게 되는 방식이 뭐였는지 떠올려야함.
    ...
    public void displayInfo3(){
        // 배열 전체를 출력하는 메서드
        System.out.println("현재 완결된 장치 목록 : ");
    ...
    public void performSpecificMethod(){
        for(Power device : deviceArray){
            if(device instanceof Computer){
                // deviceArray 내부에 있는 요소라면 기본적으로 Power 클래스의 객체이거나 혹은
                // (그런데 인터페이스는 객체 생성불가능) 이거나 혹은
                // Power 인터페이스를 상속 받은 클래스의 객체 해당합니다.
                ...
                mouse.leftClick();  // LED 클래스에 changeColor() 메서드를 정의하고,
                                    // sout("색깔을 바꿉니다")라고 한 후에 다운캐스팅을 하세요
            }else if(device instanceof LED){
                LED led = (LED) device;
                led.changeColor();
            }   // 이상의 코드가 지시사항 2번 -> Main으로 이동
        }
    }
