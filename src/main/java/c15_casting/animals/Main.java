...
/*
    ...
    참조 자료형 캐스팅(Reference Type Casting
        ...
        1. 참조 자료형이란?
            ...
            알아야 할 사항 : 연산자(Operator) instanceof -> 연산자 개념은 첫 주차에 학습
        ...
        5. 캐스팅의 장단점
            ...
            단점 :
                ...
                복잡성 증가 ; 코드 가독성이 떨어질 수 있음 -> 이렇게 바꿨다가 저렇게 바꿨다가 하기 때문인데
                    특히 업캐스팅의 경우 암시적으로 일어나기 때문에 어디서 바뀌었는지 확인을 못하고 있다가
    ...
    instanceof 연산자 : Java에서 객체의 실제 타입을 검사하는 데 사용(어떤 클래스의 객체인지를 확인하는 용도)
        ...
        형식 ;
            object(객체명) instanceof ClassName(클래스명)   -> 1+1 생각하라고 수업했음
...
public class Main {
    public static void main(String[] args) {
        ...
        animal1.makeSound();    // Dog의 makeSound() 메서드가 실행됨.
//        animal1.fetch();        // 타입이 Animal로 선언돼있기 때문에 호출할 수 없음

        System.out.println("----------animal2 부분 시작-----------");
        Animal animal2 = new Dog(); // 업캐스팅 2번 방법 -> 얘가 정처기 등 자격증 시험에 더 자주 나옵니다.
        // A a = new B();
        animal2.makeSound();    // 'Dog'의 재정의된 makeSound()가 호출
//        animal2.fetch();        // animal1때와 동일하게 호출 불가능
        ...
        Dog dog2 = (Dog) animal2;   // 다운캐스팅의 경우 명시적으로 쓰여야하기 때문에
                                    ...
                                    // int three = (int) pi; 와 같은 방식으로 작성했지만 이제는 클래스 객체 단위에서 생각할 필요가 있음
...
//        dog3.fetch();
        /*
            ...
            그 이유는 :
                애초에 Animal로 만든 것을 아무런 변형없이 강제적으로 다운캐스팅을 시도했기 때문

            이상과 같은 컴파일링 오류를 막기 위한 방식으로는 코드를 아래와 같이 작성할 수 있습니다.
         */
