...
public class Constructor {
    ...
    Constructor(int number){
        ...
        this.num = number;  // this : 해당 클래스에서 객체를 만들게 되면 객체 이름으로 대체되는 것.
                            ...
                            // 대입 연산자 '=' 전 까지는 서로 다른 개념입니다.
    }

    // 매개변수 생성자를 직접 만드세요 -> 매개변수는 String title 로 고정합니다.
    // 생성자가 호출될 때 RequiredArgsConstructor(String 매개변수를 필수로 요구하는 생성자) 가 콘솔에 찍히도록 작성하세요.
    ...
    Constructor(int num, String name){
        System.out.println("AllArgsConstructor(int, String 매개변수를 모두 요구하는 생성자)");
        this.num = num;
        this.name = name;
    }

    // field들이 전부 출력될 수 있는 method를 정의해야만 함.
    void showInfo(){
