...
public class UserService {
    /*
        ...
        주요 특징
        데이터베이스 테이블 매빙 : Entity 클래스는 데이터베이스 테이블의 구조와 일치
            -> 지난 시간에 +----------------+ 이 형태로 그렸던 표와 관련있습니다.
        피들와 컬럼 매핑 : 클래스의 필드는 데이터베이스 테이블의 컬럼과 매핑됩니다.(마찬가지로 지난 시간 수업)
        식벽자 : @Id 애너테이션을 사용하여 기본 키(prime key)를 지정합니다 -> Spring 관련 PK
        애너테이션 사용 : @Entity, @Table, @Colum 등의 애너테이션 사용 -> Lombok과 관련
        ...
        해시코드(HashCode) : 객체의 고유한 정수 값을 반환하는 메서드
            ...
            2. 재정의 필요성 : 커스텀 클래스(사용자 정의 클래스)를 만들 때, equals() 메서드를
                재정의했다면, hashCode() 메서드로 재정의할 필요가 있음 -> 두 메서드의 일관성을 이유로

            3. 규약(protocol) :
                일관성 : 같은 객체에 대해 여러 번 호출된 hashCode() 메서드는 항상 동일한 정수값을 반환해야 함.
