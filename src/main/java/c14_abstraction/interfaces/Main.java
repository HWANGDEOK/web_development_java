...
/*
    인터페이스(interface)
        ...
        인터페이스는 메서드의 시그니처(메서드의 이름, 리턴 타입, 매개변수 목록)만을 포함하며, 메서드의 실체
            구현은 포함하지 않음 -> 마치 추상 메서드와 비슷해 보임
        다중 상속을 지원 -> 이를 통해 클래스가 여러 인터페이스를 구현할 수 있음 :
            일반 클래스의 경우 하나의 클래스만 상속가능

    특징
        1. 추상 메서드 : 인터페이스 내의 모든 메서드는 기본적으로 추상 메서드 -> 즉, 메서드의 정의만 있고
            구현은 없다.
        2. 상수 : 인터페이스 내에서 선언된 벼수는 '자동으로' public static final 상수로 취급됨.
    ...
    추상 클래스와의 차이점
       ...
       - 인터페이스
            ...
            2. 인터페이스는 인스턴스 변수(필드 중 객체마다 값이 달라지는 변수)를 가질 수 없고, 상수만 선언가능
            3. 다중 상속 : 클래스는 여러 인터페이스를 구현할 수 있다.
            4. 생성자 없음 : 인터페이스는 생성자를 가질 수 없다. -> 일단 맴버 변수(인스턴스 변수)에서 없기
                때문에 생성자 생성 불가능 -> 객체마다 다른 값을 가질 수 없다는 의미가 됩니다.
            5. 자동 public : 인터페이스의 메서드들은 자동으로 public 이며 , 메서드 선언에 접근 제어자를 명시할 필요가 없다.
                Java 8 이후에, default, static 메서드를 사용할 수 있긴 합니다.
...
public class Main {
    public static void main(String[] args) {
        ...
        /*
            ...
            AllArgsConstructor이므로 오류가 발생
            그렇다면 내부에 들어가야 할 매개변수를 채워야 할 필요가 있다

            여기서 여태까지와 다른 방식으로 argument를 채워넣기 때문에 유의해야함

            구조를 이해해야 함
            RemoteController 의 객체를 생성했는데, 그 객체 이름이 remoteController입니다
            클래스1명 객체명 = new 생성자(argument1, argument2, ...)
            클래스1명 객체명 = new 1명(new 클래스2명, new 클래스3명)

            이상의 경우 new 클래스2명(), new 클래스3명()를 통해 객체 생성은 이미 완료가 되었습니다.
            다만, 여태까지와의 차이점은 new 클래스2명(), new 클래스3명()에 해당하는 객체명이 없을 뿐입니다.
         */
//        RemoteController remoteController = new RemoteController(new PowerButton(), new ChannelUpButton(),new ChannelDownButton(), new VolumeDownButton(), new VolumeUpButton());
//
...
/*
    ...
    2. RemoteController.java의 필드에 의해서 만든 클래스들의 객체를 추가하시오.
    ...
    Main 에 AirConditionerController의 객체를 생성한 후
