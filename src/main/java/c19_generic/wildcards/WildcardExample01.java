...
/*
    1. 와일드 카드란 무엇인가?
        ...
        자바에서 와일드카느는 물음표(?)로 표현되며, 이를 통해 특정 타입을 고정하지 않고 타입의 범위를 제한하거나 열어둘 수 있음.

    2. 와일드 카드의 필요 이유
        1) 다양한 타입의 데이터를 처리하기 위해 제네릭 클래스나 메서드 특정 타입에 제한적일 수 있음.
...
public class WildcardExample01 {
    // 이하의 메서드가 매개변수로 List 타입을 받는데, List 내부의 요소의 자료형을 제한두지 않도록 와일드 카드를 설정함.
